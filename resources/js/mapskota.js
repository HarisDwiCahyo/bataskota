...
// Fungsi untuk Mendapatkan Data GeoJSON Berdasarkan Layer yang Dipilih
...
// Ambil data GeoJSON dari endpoint Anda
// Global variable for storing highlighted feature
var highlightedFeature = null;

// Function to create a VectorLayer from GeoJSON data
...
// Function to add the vector layer to the map
function addLayerToMap(vectorLayer) {
    overlays.getLayers().push(vectorLayer);
}

// Function to fit the map view to the extent of the features
function fitMapToExtent(vectorSource) {
    var extent = vectorSource.getExtent();
    map.getView().fit(extent, { duration: 1000 });
}

// Function to create a highlight style for features
...
function handleMapClick(vectorLayer, highlightStyle) {
...
// Function to display feature properties in the UI
...
function initializeMap() {
    $.getJSON("/admin/kota", function (response) {
        if (response.data) {
            var { vectorSource, vectorLayer } = createVectorLayer1(
                response.data
            );
            addLayerToMap(vectorLayer);
            fitMapToExtent(vectorSource);

            var highlightStyle = createHighlightStyle();
            handleMapClick(vectorLayer, highlightStyle);
        } else {
